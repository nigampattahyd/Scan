...
namespace BussinessLogicLayer
{
    public class PDFtoImage
    {
        ...
        {
            string strpats = destinationPngFilePath + ".txt";             
            try
            {               
                ...
                GhostscriptWrapper.GenerateOutput(sourcePdfFilePath, destinationPngFilePath,
                    ...
                    {
                        Device = GhostscriptSharp.Settings.GhostscriptDevices.pngalpha,
                ...
                StreamWriter writer = File.CreateText(strpats);
        ...
        public void ConvertPdfImage(string sourcePdfFilePath, string destinationPngFilePath)
        {
            ...
            try
            {
                ...
                //_rasterizer.Open(sourcePdfFilePath, _lastInstalledVersion, false);
                //Image img = _rasterizer.GetPage(desired_x_dpi, desired_y_dpi, 0);
                //img.Save(destinationPngFilePath, ImageFormat.Png);
                ...
                str.AppendLine("-------------------Success-------------");
                str.AppendLine("-------------------END-------------" + DateTime.Now);
            }
            catch (Exception ex)
            {
                ex.ToString();
                str.AppendLine("-------------------Exception START-------------" + DateTime.Now);
                str.AppendLine(ex.ToString());
                str.AppendLine("-------------------Exception END-------------" + DateTime.Now);    
            }
